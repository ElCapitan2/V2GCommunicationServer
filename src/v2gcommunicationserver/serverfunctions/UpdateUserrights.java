...
/**
 * Server side implementation to process a request update user rights
 * 
...
public class UpdateUserrights implements TaskWorker{
     ...
     * DatabaseConnection and update the user rights
     * 
     * It will build Answer Request, indicating whether the userrights where 
    ...
    public void run() {
        if (request.getRequestType()==RequestType.REQUEST){
            ...
            boolean userCreated;
            userCreated = conn.updateUserrights(newUsername, 
    ...
    /**
     * Builds an Answer request indicating whehter the userrights where updated 
     * successfully
     * 
     * @param username              String username to send the Request to
     * @param ID                    Request ID
     * @param userrightsUpdated     Success useright update
     * @param nameOfCreatedUser     Name of the user
     * @return                      Request to be transmitted
     */
    private Request buildAnswerRequest(String username, String ID, boolean userrightsUpdated, String nameOfCreatedUser){
        ParameterSet psAnswer = new ParameterSet();
        psAnswer.parameterName = "userrightsUpdated";
        psAnswer.parameterType = boolean.class.getName();
        psAnswer.parameterValue = String.valueOf(userrightsUpdated);
        psAnswer.timeStamp = null;
        
        ParameterSet psNameOfCreateUser = new ParameterSet();
        psNameOfCreateUser.parameterName = "nameOfUser";
